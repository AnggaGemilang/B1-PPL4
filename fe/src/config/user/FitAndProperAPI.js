...
export default {
  ...
  nilai: (id, data) => api.put(`/scores/${id}`, data).then((res) => res.data),
  delete: (id) => api.delete(`/mappings/${id}`).then((res) => res.data),

  getRekapManualFitProper : (registrant, projection) => api.get(`/line-mappings?populate[1]=mapping&populate[2]=mapping.position&populate[3]=scores_fitproper&populate[4]=scores_fitproper.criterion&populate[5]=examiner.employee&populate[6]=mapping.registrant.employee&populate[7]=mapping.level&filters[mapping][registrant][id][$eq]=${registrant}&filters[mapping][position][id][$eq]=${projection}`).then((res) => res.data),
  getRekapManualWawancara : (registrant, projection) => api.get(`/line-mappings?populate[1]=mapping&populate[2]=mapping.position&populate[3]=scores_interview&populate[4]=scores_interview.criterion&populate[5]=examiner.employee&populate[6]=mapping.registrant.employee&populate[7]=mapping.level&filters[mapping][registrant][id][$eq]=${registrant}&filters[mapping][position][id][$eq]=${projection}&filters[is_interview][$eq]=true`).then((res) => res.data),
